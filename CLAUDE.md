...
- **Go API**：JWT 无状态认证。access token + refresh token，请求头 `Authorization: Bearer <token>`。登出时 token 加入 Redis 黑名单（`blacklist:` 前缀）。
- **维护模式**：数据库恢复时通过 Redis 键 `system:maintenance` 阻止请求。PHP `Auth` 中间件和 Go JWT 中间件均检查此键，超管（id=1）不受影响。
